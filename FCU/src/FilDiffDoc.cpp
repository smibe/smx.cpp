...
bool CFilDiffDoc::CompareFiles()
{
  ...
  m_mergeEdit.SetNotifyObject(this);
	m_mergeEdit.m_pOnFoldedLines = (LineChgFP) OnFoldedLines;
  m_mergeEdit.m_pOnUnFoldLine = (LineChgFP) OnUnFoldLines;
...
bool CFilDiffDoc::GetRefSect(int& nIdxStart, PLINEDESC pLine)
{
	...
	for(int i=nIdxStart; i <= m_secDiffList.GetUpperBound(); i++)
	{
...
UINT AFXAPI HashKey( CTextSect TextSect)
{
  UINT nHash = 0;
