...
IMPLEMENT_DYNCREATE(CDiffDirFrame, CFcuMDIChildWndEx)
...
BEGIN_MESSAGE_MAP(CDiffDirFrame, CFcuMDIChildWndEx)
...
{
	CFcuMDIChildWndEx::OnMDIActivate(bActivate, pActivateWnd, pDeactivateWnd);
}
...
{
	if (CFcuMDIChildWndEx::OnCreate(lpCreateStruct) == -1)
		return -1;
...
void CDiffDirView::OnContextMenu(CWnd* pWnd, CPoint point)
{
	...
	*/
	...
	Menu.TrackMenu(this, point, (UpdateMenuFkt)&CDiffDirView::OnUpdatePopup);
}
...
void CDiffDirView::ExecuteShellCmd(LPCTSTR pszCmd, bool bRight, bool bRelative)
{
	...
	{
		if (_stricmp(pszCmd, szOpen) == NULL)
		{
		...
		else if (_stricmp(pszCmd, local::szDelete) == NULL)
		{
			CShellFileOp op;
...
{
	ExecuteShellCmd(local::szDelete, false, true);
}

void CDiffDirView::OnDelete_R()
{
	ExecuteShellCmd(local::szDelete, true, true);
}
