...
BOOL SECEdit::InsertAllocLines(PLINEDESC& pBef, PLINEDESC& pAfter, PLINEDESC pStart,PLINEDESC pEnd)
{
  ...
  {
    m_pLine = pStartCopy;
    pStartCopy->pPrev = NULL;
  }

  if(pBef)
  {
    pEndCopy->pNext = pBef;
    pStartCopy->pPrev = pBef->pPrev;
    pBef->pPrev = pEndCopy;
		if(pStartCopy->pPrev)
			pStartCopy->pPrev->pNext = pStartCopy;
		else
			m_pLine = pStartCopy;
  }
  ...
  if(pAfter)
  {
    pEndCopy->pNext = pAfter->pNext;

    if(pAfter->pNext)
			pAfter->pNext->pPrev = pEndCopy;

    pStartCopy->pPrev = pAfter;
    pAfter->pNext = pStartCopy;
  }
