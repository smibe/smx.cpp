...
PLINEDESC SECEdit::AllocCopyLine(PLINEDESC pLine)
{
   PLINEDESC pNewLine;
   int iRequest = pLine->iBuffLen;
   int iTextSize = strlen(pLine->pText);
   ...
   if (pNewLine && iTextSize)
   {
      ...
      strcpy(pNewLine->pText, pLine->pText);

      pNewLine->iBuffLen = iRequest;
		  pNewLine->iLineNo = 1;
      pNewLine->iTxtSize = pLine->iTxtSize;
      pNewLine->flags = pLine->flags;
      pNewLine->state = pLine->state;

      // outgoing text always gets set to OE_CRLF
      if (FindLineEnd(pNewLine->pText, pNewLine->iTxtSize) != OE_CRLF)
        ConvertLineEnds(pNewLine, OE_CRLF);

   }
   return pNewLine;
}

PLINEDESC SECEdit::AllocLine(const CString& str)
{
   int iRequest = str.GetLength() + 5;
   int iTextSize = str.GetLength();

   PLINEDESC pNewLine = (PLINEDESC)SECEditAlloc(sizeof(LINEDESC));
   if (pNewLine && iTextSize)
   {
      /* round up to the next highest multiple of 32 */
      iRequest = ((iTextSize+31) >> 4) << 4;
      pNewLine->pText = (LPSTR)SECEditAlloc(iRequest);
      strcpy(pNewLine->pText, str);

      pNewLine->iBuffLen = iRequest;
		  pNewLine->iLineNo = 1;
      pNewLine->iTxtSize = str.GetLength();
      pNewLine->flags = 0;
      pNewLine->state = 0;
