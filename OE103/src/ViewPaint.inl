...
void SECEditCore<BC>::DrawMarks(CPaintDC* dc, RECT* rect, PLINEDESC pLine)
{
	...
	int iMarkSpace = m_iMarkSpace - m_iLineNumSpace;
  if (pLine && pLine->fBookMark)
		{
		temprect.top = rect->top;
		temprect.left = 2;
		temprect.right = ((iMarkSpace/2)*2)-2;
  ...
  {
    //We should paint line numbers here
    CRect rcLineNum = rect;
    rcLineNum.left += iMarkSpace;
    PaintLineNumbers(dc, pLine, rcLineNum);
  }
}
   ...
   while (Y < bottom)
   {
      if ((Y+m_iLineHeight) > top)
      {
         ...
         temprect.bottom = rect.bottom;
			...
			temprect.right -= m_iLineNumSpace;
...
template <class BC>
void SECEditCore<BC>::PaintLineNumbers(CDC *pDC, PLINEDESC pLine, CRect rcLineNum)
{
  CString str;

  pDC->FillSolidRect(rcLineNum, pDC->GetBkColor());

  if (!pLine || SECEdit::sm_nFoldedState && pLine->state == (UINT) SECEdit::sm_nFoldedState)
  {
    //Folded lines have no line number
    return;
  }
  
  COLORREF crBkg = pDC->GetBkColor();
  COLORREF crTxt = pDC->GetTextColor();
  
  //Note that we allow GetDisplayLineNo to change colors
  int nDisplayLineNo = GetDisplayLineNo(pLine, pDC);
  if (nDisplayLineNo > 0)
  {
    str.Format(_T("%d "), nDisplayLineNo);
    pDC->DrawText(str, rcLineNum, DT_RIGHT);
  }

  pDC->SetBkColor(crBkg);
  pDC->SetTextColor(crTxt);
  //pDC->ExtTextOut(rcLineNum.left, rcLineNum.top, ETO_OPAQUE, rcLineNum, str, NULL);
}

template <class BC>
int SECEditCore<BC>::GetDisplayLineNo(PLINEDESC pLine, CDC *pDC)
{
