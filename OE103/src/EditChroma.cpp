...
void SECEdit::SetFileMappingKey(LPCTSTR pszSection)
   {
  if (pszSection)
...
LPCTSTR SECEdit::GetFileMappingKey()
{
	return m_strSection;
}
...
BOOL SECEdit::GetKeyWords()
   ...
   if (h != INVALID_HANDLE_VALUE)
      ...
      glpMyIniFileBuffer = new char[giMyIniLength+32];
  ...
  lpTemp = new char[dwSize];
   ...
   lpNames = new CKEYWORD[KEYWORDARRAYSIZE];
	if (!lpNames)
		{
		m_bChroma = FALSE;
			delete [] lpTemp;
