...
//250698:GeH:AllocCopyLine() added
...
typedef void (CObject::*LineChgFP)(SECEdit *pEdit, PLINEDESC pLine);


//Special notification messages
#define EN_VSCROLLCHANGED 0x0702
...
public:
	...
	void InitEdit(LPCTSTR pszSection);
   ...
   int GetLineEndLength(void);
    ...
    void ConvertLineEnds(PLINEDESC lpText, int iLineEndType);
   ...
   //GeH: new functions	
   //@cmember CutLines deletes the lines
   BOOL CutLines(PLINEDESC pStart, PLINEDESC pEnd);
   
  PLINEDESC AllocLine(const CString& str);

   //@cmember Free the memory of the specified lines.
   BOOL FreeLines(PLINEDESC pStart, PLINEDESC pEnd = NULL);
   BOOL DeleteLines(PLINEDESC pStart, PLINEDESC pEnd = NULL);
   
   //@cmember Detach the lines
   PLINEDESC DetachLines(PLINEDESC pStart, PLINEDESC pEnd);

   BOOL InsertLines(PLINEDESC& pBef, PLINEDESC& pAfter, PLINEDESC pStart,PLINEDESC pEnd = NULL);
    PLINEDESC FoldLines(PLINEDESC pStart, PLINEDESC pEnd, LPCTSTR pszFoldedText);
    PLINEDESC UnFoldLines(PLINEDESC pStart, bool bSaveUndoRecord = false);
    PLINEDESC CreateFoldedLine(int iStart, int iEnd, LPCTSTR pszFoldedText);
    void      UnFoldAll();
    void      Unlink(PLINEDESC pLine);

   	//@cmember Inserts lines at the given position.
   BOOL InsertAllocLines(PLINEDESC& pBef, PLINEDESC& pAfter, PLINEDESC pStart,PLINEDESC pEnd);
    ...
    LPSTR ExpandTabs(LPSTR lpText,int iTextSize, int *iReturnSize);
	...
	virtual int JoinLines(PTEXTPOS pTextPos, BOOL bSaveUndoRecord);
	...
	void SetFileMappingKey(LPCTSTR pszSection);

	//@cmember Internal function that maintains a file mapping table.
	void SetFileMappingKey();
	LPCTSTR GetFileMappingKey();
	...
	//GeH
	//@cmember Internal function used to Copy lines.
	PLINEDESC AllocCopyLine(PLINEDESC pLine);

   //@cmember Attach a new line list, delete the old one. (GeH)	
   void AttachLineList(PLINEDESC pFirstLine);
  
   void SetNotifyObject(CObject *pObj) { ASSERT(m_pObj == NULL); m_pObj = pObj; }
   PLINEDESC GetFoldedLineList(PLINEDESC pFoldedLine);
...
public:
  static LPCTSTR sm_szKeepTabs;
  static LPCTSTR sm_szShowTabs;
  static LPCTSTR sm_szWordWrap;
  static LPCTSTR sm_szShowLineNo;
  static LPCTSTR sm_szTabSize;
  static LPCTSTR sm_szFontName;
  static LPCTSTR sm_szFontWeight;
  static LPCTSTR sm_szFontSize;
  static LPCTSTR sm_szFontItalic;
  static LPCTSTR sm_szFontCharset;
...
private:
   ...
   int LineListFree(PLINEDESC pFirstLine);
...
// Attributes
public:
  static int sm_nFoldedState;
  static int sm_nUnFoldedState;
  ...
  //Special change notification handling
  CObject   *m_pObj;
  LineChgFP  m_pOnFoldedLines;    //Called after lines were folded
  LineChgFP  m_pOnUnFoldLine;     //Called before lines are unfolded
...
public:
  ...
  void SetAccelTableID(UINT  nID) { m_nAccelTableID = nID; }
...
public:
  ...
  virtual void PaintLineNumbers(CDC *pDC, PLINEDESC lpLine, CRect rcLineNum);
  ...
  void InvalidateLineRect(PLINEDESC pLine);
  ...
  void SetPaintLineNumbers(bool bPaint);
  void CalcLineNumSpace();
  bool GetPaintLineNumbers()            { return m_iLineNumSpace > 0; }
   ...
   virtual int Paste(BOOL bUpdate, BOOL bSaveUndoRecord);
  ...
  //@cmember Return true if something is selected
	virtual BOOL HasSelection();    //GeH

	//@cmember Check if the empty line is in a selected block.
	BOOL InBlockEMptyLine(PLINEDESC pstTempLine);
...
public:
   ...
   int          m_iMarkSpace;           //
	...
	//@cmember Report VScrollChanged notification.
  bool       m_bSendVScrollChanged;

  int  m_iLineNumSpace;       //Space needed for painting the line numbers.
