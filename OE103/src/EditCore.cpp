...
LPCTSTR SECEdit::sm_szKeepTabs    = _T("Keep  tabs");
LPCTSTR SECEdit::sm_szShowTabs    = _T("Show tabs");
LPCTSTR SECEdit::sm_szWordWrap    = _T("Word wrap");
LPCTSTR SECEdit::sm_szShowLineNo  = _T("Show Line No");
LPCTSTR SECEdit::sm_szTabSize     = _T("Tab size");
LPCTSTR SECEdit::sm_szFontName    = _T("Font name");
LPCTSTR SECEdit::sm_szFontWeight  = _T("Font weight");
LPCTSTR SECEdit::sm_szFontSize    = _T("Font size");
LPCTSTR SECEdit::sm_szFontItalic  = _T("Font italic");
LPCTSTR SECEdit::sm_szFontCharset = _T("Char set");
...
SECEdit::~SECEdit()
   {
	...
	ClearEdit();
    PLINEDESC pLines = NULL;
    PLINEDESC pKey   = NULL;
  
    for (POSITION pos = m_FoldedMap.GetStartPosition(); pos; )
    {
      m_FoldedMap.GetNextAssoc(pos, (void *&) pKey, (void *&) pLines);
      if (pLines)
      {
        SECEdit::DeleteLines(pLines, NULL);
      }
    }
    m_FoldedMap.RemoveAll();
   }

//@doc SECEdit
//@mfunc Internal function. Initializes SECEdit data items
//@rdesc void 
void SECEdit::InitEdit(LPCTSTR pszSection)
	{
   lpNames = NULL;
  ...
  m_pObj  = NULL;
  m_pOnFoldedLines = NULL;
  m_pOnUnFoldLine = NULL;
	}
...
int SECEdit::InitFileTypeSettings()
   ...
   InitChroma();   // re-read the chroma stuff
	...
	m_iTabSize = AfxGetApp()->GetProfileInt(m_strSection,sm_szTabSize,3);
	m_bKeepTabs = AfxGetApp()->GetProfileInt(m_strSection,sm_szKeepTabs,1);
	m_bShowTabs = AfxGetApp()->GetProfileInt(m_strSection,sm_szShowTabs,0);
	m_fontname = AfxGetApp()->GetProfileString(m_strSection,sm_szFontName,_T("Courier New"));
	m_fontsize = AfxGetApp()->GetProfileInt(m_strSection,sm_szFontSize,9);
	m_fontcharset = AfxGetApp()->GetProfileInt(m_strSection,sm_szFontCharset,0);
	m_fontweight = AfxGetApp()->GetProfileInt(m_strSection,sm_szFontWeight,400);
	m_fontitalic = AfxGetApp()->GetProfileInt(m_strSection,sm_szFontItalic,0);

	m_bWordWrap = AfxGetApp()->GetProfileInt(m_strSection,sm_szWordWrap,0);
  m_bShowLineNumbers = AfxGetApp()->GetProfileInt(m_strSection,sm_szShowLineNo,0) != 0;
...
LPSTR SECEdit::ExpandTabs(LPSTR lpText,int iTextSize, int *iReturnSize)
{
  return ExpandTabs(lpText, iTextSize, m_iTabSize, m_iLineEndLength, iReturnSize);
}
...
void SECEdit::SetShowLineNumbers(bool bShow)
{
  m_bShowLineNumbers = bShow;
}

bool SECEdit::GetShowLineNumbers()
{
...
PLINEDESC SECEdit::GetFoldedLineList(PLINEDESC pFoldedLine)
{
  PLINEDESC pLine = NULL;
  m_FoldedMap.Lookup(pFoldedLine, (void *&) pLine);
  return pLine;
}
