...
BOOL SECEdit::InsertAllocLines(PLINEDESC& pBef, PLINEDESC& pAfter, PLINEDESC pStart,PLINEDESC pEnd)
{
  PLINEDESC pStartCopy = NULL;
  PLINEDESC pEndCopy = NULL;
  PLINEDESC pLine = pStart;
  PLINEDESC pPrev = NULL;

  //Copy the lines.
  while(pLine)
  {
    pEndCopy = AllocCopyLine(pLine);

    //Update the links
    ASSERT(pLine->link == NULL || pLine->link == pLine);
    pLine->link = pEndCopy;
    pEndCopy->link = pLine;

    if(pPrev)
    {
      pEndCopy->pPrev = pPrev;
      pPrev->pNext = pEndCopy;
    }
  ...
  //Now link them in.
  return InsertLines(pBef, pAfter, pStartCopy, pEndCopy);
}

BOOL SECEdit::InsertLines(PLINEDESC& pBef, PLINEDESC& pAfter, PLINEDESC pStart,PLINEDESC pEnd)
{
  if (!pEnd)
  {
    pEnd = pStart;
  ...
  if(pBef)
  {
    ...
    pStart->pPrev = pBef->pPrev;
    pBef->pPrev = pEnd;
		if(pStart->pPrev)
			pStart->pPrev->pNext = pStart;
		else
			m_pLine = pStart;
  }
  else if(pAfter)
  {
    pEnd->pNext = pAfter->pNext;

    if(pAfter->pNext)
			pAfter->pNext->pPrev = pEnd;
