...
int SECEdit::CountLines(PLINEDESC pLine)
{
  ...
  int iDisplayNumber = 1;
  for (i = 0;pTempLine;pTempLine = pTempLine->pNext)
  {
    i++;
    pTempLine->iLineNo = i;
    
    if (sm_nFoldedState && pTempLine->state == (UINT) sm_nFoldedState)
    {
      PLINEDESC pTemp = NULL;
      m_FoldedMap.Lookup(pTempLine, (void *&) pTemp);
      pTempLine->iDisplayLineNo = iDisplayNumber;
      while (pTemp && pTemp->pNext)
      {
        pTemp->iDisplayLineNo = iDisplayNumber;
        iDisplayNumber++;
        pTemp = pTemp->pNext;
      }
      if (pTemp)
...
void SECEdit::ConvertLineEnds(PLINEDESC lpText, int iLineEndType)
{
  int nLineEnd    = 0;               //number of line end characters found.
  LPTSTR pBuffer  = lpText->pText;
  TCHAR szNewLineEnd[4];
  szNewLineEnd[0] = 0x0D;
  szNewLineEnd[1] = 0x0A;
  szNewLineEnd[2] = 0;
  ...
  int iNewTextSize = strlen(pBuffer);

  //Reallocate teh buffer if needed
  if (iNewTextSize >= lpText->iBuffLen)
  {
    int iRequest = ((iNewTextSize+15) >> 4) << 4;
    /* add a little fudge */
    iRequest += 32;

    LPTSTR pTemp = new TCHAR[iRequest];
    strcpy(pTemp, pBuffer);
    delete [] pBuffer;
    pBuffer = pTemp;
    lpText->iBuffLen = iRequest;
    lpText->pText = pTemp;
  }

  //Add the end of line characters
  strcat(pBuffer, szNewLineEnd);
  lpText->iTxtSize = strlen(lpText->pText);
}

void SECEdit::AttachLineList(LINEDESC *pFirstLine)
{
	FreeLineList(m_pLine);
	m_pLine = pFirstLine;
}