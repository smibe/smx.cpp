...
bool CRegistry::WriteBinary (LPCTSTR pszKey, CObject& rObject)
{
	ASSERT(m_hKey);
	ASSERT(pszKey);
	const int iMaxChars = 2048;
	DWORD dwData = iMaxChars;
	BYTE* byData = (BYTE*)::calloc(iMaxChars, sizeof(TCHAR));
	ASSERT(byData);

	CMemFile file(byData, iMaxChars, 16);
	CArchive ar(&file, CArchive::store, 1024);
	ASSERT(rObject.IsSerializable());
	rObject.Serialize(ar);
	ar.Close();
...
bool CRegistry::ReadBinary (LPCTSTR pszKey, CObject& scStringList)
{
	ASSERT(m_hKey);
	ASSERT(pszKey);
