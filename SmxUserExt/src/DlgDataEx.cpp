...
void DDX_CheckRegistry(CDataExchange* pDX, int nIDC, LPCTSTR pszSection, LPCTSTR pszKey, bool bDefault)
{
  int nCheck = bDefault;
  
  if (!pDX->m_bSaveAndValidate)
    AfxGetApp()->GetProfileInt(pszSection, pszKey, nCheck); 

  DDX_Check(pDX, nIDC, nCheck);

  if (pDX->m_bSaveAndValidate)
    AfxGetApp()->WriteProfileInt(pszSection, pszKey, nCheck); 
}
