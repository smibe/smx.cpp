...
/***********************************************************
  ...
  Latest Changes:
		...
		11/22/2001	--	Fixed bug causing file name's to be truncated if
						...
						is first called.	See docuementation API for SmxReadDirectoryChangesW() for more information of required privileges.
		...
		11/23/2001		Added classes so that all notifications are handled by the
						...
						1: The worker thread spends less time handling each notification.
							...
							and SmxReadDirectoryChangesW() can be called again to receive more notifications
							faster.
		...
		02/06/2002	  Fixed a bug that would cause an application to hang.
						If SmxReadDirectoryChangesW was to fail during normal operation,
						...
						3)  Change the IP address of 'ThisComputer'
							   ...
							   ** SmxReadDirectoryChangesW() will fail some point after this.
...
//	to use the SmxSmxReadDirectoryChangesW() API.  See documentation.
{    
	BOOL fOk = FALSE;    
...
by SmxSmxReadDirectoryChangesW().
...
FILE_NOTIFY_INFORMATION is defined in Winnt.h as: 
 ...
 typedef struct _FILE_NOTIFY_INFORMATION {
  ...
  SmxSmxReadDirectoryChangesW basically puts x amount of these records in a 
  ...
  ...
  SmxSmxReadDirectoryChangesW(...Read_Buffer, 4096,...);
  ...
...
CPrivilegeEnabler::CPrivilegeEnabler()
{
	LPCTSTR arPrivelegeNames[]	=	{
										...
										SE_RESTORE_NAME,//  CreateFile() to open the directory handle for SmxReadDirectoryChangesW
...
void CDirectoryChangeHandler::On_ReadDirectoryChangesError(DWORD dwError, const CString & strDirectoryName)
{
	...
	TRACE(_T("SmxReadDirectoryChangesW has failed! %d"), dwError);
...
void CDirectoryChangeHandler::On_FileNotifyFinished()
{ 
	TRACE(_T("File nofifications finished"));
}
...
FUNCTION:	WatchDirectory(const CString & strDirToWatch,   --the name of the directory to watch
						   DWORD dwChangesToWatchFor, --the changes to watch for see dsk docs..for SmxReadDirectoryChangesW
...
PARAMETERS:
		...
		DWORD dwChangesToWatchFor	-- specifies flags to be passed to SmxReadDirectoryChangesW()
	...
	from the initial call to SmxReadDirectoryChangesW().  It's also possible to get an
...
**************************************************************/
{
	...
	//	so that it has required permissions to use the SmxReadDirectoryChangesW API, etc.
	//
	...
	{//completion port created/directory associated w/ it successfully
		...
		//when the thread starts, it will call SmxReadDirectoryChangesW and wait 
		...
		{//thread is running, 
		   ...
		   {//SmxReadDirectoryChangesW was successfull!
...
//
//	Called in the worker thread in the case that SmxReadDirectoryChangesW() fails
...
/*********************************************
  Sets the running state of the object to perform the initial call to SmxReadDirectoryChangesW()
  , wakes up the thread waiting on GetQueuedCompletionStatus()
  and waits for an event to be set before returning....

  The return value is either ERROR_SUCCESS if SmxReadDirectoryChangesW is successfull,
  or is the value of GetLastError() for when SmxReadDirectoryChangesW() failed.
**********************************************/
{
	ASSERT( hCompPort );
	...
	return m_dwReadDirError;//This value is set in the worker thread when it first calls SmxReadDirectoryChangesW().
}
...
//						and abnormally by the worker thread in the case that SmxReadDirectoryChangesW fails -- see code.
//
...
********************************************/
{
    ...
    do
    {
		...
		{//The io completion request failed...
		 ...
		 //was used in a call to SmxReadDirectoryChangesW has been closed.
			...
			if( bObjectShouldBeOk )
			{
				...
				************************************/
				...
				case CDirWatchInfo::RUNNING_STATE_START_MONITORING:
					{
						//Issue the initial call to SmxReadDirectoryChangesW()
						
						if( !SmxReadDirectoryChangesW( pdi->m_hDir,
						...
						pdi->m_StartStopEvent.SetEvent();//signall that the SmxReadDirectoryChangesW has been called
														 //check CDirWatchInfo::m_dwReadDirError to see whether or not SmxReadDirectoryChangesW succeeded...
				...
				case CDirWatchInfo::RUNNING_STATE_STOP:
					{
						...
						{
						 //Since I've previously called SmxReadDirectoryChangesW() asynchronously, I am waiting
						 //for it to return via GetQueuedCompletionStatus().  When I close the
						 //handle that SmxReadDirectoryChangesW() is waiting on, it will
						 ...
						 //get called again
							...
							pdi->m_RunningState = CDirWatchInfo::RUNNING_STATE_STOP_STEP2;//back up step...GetQueuedCompletionStatus() will still need to return from the last time that SmxReadDirectoryChangesW() was called.....
						...
						else
						{
							...
							pdi->m_StartStopEvent.SetEvent();//set the event that SmxReadDirectoryChangesW has returned and no further calls to it will be made...
						}
				...
				case CDirWatchInfo::RUNNING_STATE_STOP_STEP2:
					{
						...
						//time that SmxReadDirectoryChangesW was called...
						//Using CloseHandle() on the directory handle used by
						//SmxReadDirectoryChangesW will cause it to return via GetQueuedCompletionStatus()....
						if( pdi->m_hDir == INVALID_HANDLE_VALUE )
							pdi->m_StartStopEvent.SetEvent();//signal that no further calls to SmxReadDirectoryChangesW will be made
				...
				case CDirWatchInfo::RUNNING_STATE_NORMAL:
					{
											...
											NULL) )//no completion routine!
						{
							...
							//		the I/O completion port...there will be no more outstanding calls to SmxReadDirectoryChangesW
							...
							//
							TRACE(_T("WARNING: SmxReadDirectoryChangesW has failed during normal operations...failed on directory: %s\n"), pdi->m_strDirName);
...
/////////////////////////////////////////////////////////////
{
	...
	//									returned by SmxReadDirectoryChangesW)
	//
    do
	{
		...
		case FILE_ACTION_RENAMED_OLD_NAME:
			{//a file name has changed, and this is the OLD name
			 ...
			 //the action set to FILE_ACTION_RENAMED_NEW_NAME (contains the new name of the file
				...
				{
					//this OLD_NAME was the last record returned by SmxReadDirectoryChangesW
					//I will have to call SmxReadDirectoryChangesW again so that I will get 
					//the record for FILE_ACTION_RENAMED_NEW_NAME

					//Adjust an offset so that when I call SmxReadDirectoryChangesW again,
					...
					that SmxReadDirectoryChangesW will have to report to
					me.   There might not be enough room in the buffer
					and the last record that we DID get was an OLD_NAME record,
					I will need to call SmxReadDirectoryChangesW again so that I will get the NEW_NAME 
				   ...
				   our read buffer would look something like this:
					...
					to the beginning of the buffer used by SmxReadDirectoryChangesW()
					...
					SmxReadDirectoryChangesW(..., pBuffer + (an Offset),
  ...
  pChangeHandler->On_FileNotifyFinished();
}