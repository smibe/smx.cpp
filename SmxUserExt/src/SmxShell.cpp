...
CSMXShell::CSMXShell(LPCTSTR lpszFileName)
{
...
int CALLBACK CSMXShell::BrowseCtrlCallback(HWND hwnd, UINT uMsg, LPARAM /*lParam*/, LPARAM lpData)
{
	CSMXShell* pCJShellInfo = (CSMXShell*)lpData;
...
BOOL CSMXShell::BrowseForFolder(CWnd *pParentWnd)
{
	ASSERT(pParentWnd);
...
void CSMXShell::GetSystemImageList(CImageList *pSmallList, CImageList *pLargeList)
{
	ASSERT(pSmallList);
	ASSERT(pLargeList);
...
CString CSMXShell::GetFileName(LPCTSTR lpszFileName/*=NULL*/)
{
	// Parse the file path.
...
CString CSMXShell::GetFileDrive(LPCTSTR lpszFileName/*=NULL*/)
{
	// Parse the file path.
	if (lpszFileName != NULL) {
		SplitPath(lpszFileName, m_szDrive, m_szDir, m_szFname, m_szExt);
...
CString CSMXShell::GetRoot(LPCTSTR lpszFileName/*=NULL*/)
{
	// Parse the file path.
	if (lpszFileName != NULL) {
		SplitPath(lpszFileName, m_szDrive, m_szDir, m_szFname, m_szExt);
...
CString CSMXShell::GetFileTitle(LPCTSTR lpszFileName/*=NULL*/)
{
	// Parse the file path.
	if (lpszFileName != NULL) {
		SplitPath(lpszFileName, m_szDrive, m_szDir, m_szFname, m_szExt);
...
CString CSMXShell::GetFileExt(LPCTSTR lpszFileName/*=NULL*/)
{
	// Parse the file path.
	if (lpszFileName != NULL) {
		SplitPath(lpszFileName, m_szDrive, m_szDir, m_szFname, m_szExt);
...
CString CSMXShell::GetFileType(LPCTSTR lpszFileName/*=NULL*/)
{
	SHFILEINFO	sfi;
...
CString CSMXShell::GetDisplayName(LPCTSTR lpszFileName/*=NULL*/)
{
	SHFILEINFO sfi;
...
int CSMXShell::GetIconIndex(LPCTSTR lpszFileName/*=NULL*/, UINT uFlags/*=SHGFI_SMALLICON*/)
{
	SHFILEINFO sfi;
...
BOOL CSMXShell::Exist(LPCTSTR lpszFileName/*=NULL*/)
{
	WIN32_FIND_DATA fd;
...
CString CSMXShell::GetLastWriteTime(CTime *pTime)
{
	if (!pTime)
		return _T("");
...
CString CSMXShell::GetFileSizeString(LPCTSTR lpszFileSize)
{
...
LPITEMIDLIST CSMXShell::Next(LPCITEMIDLIST pidl)
{
...
UINT CSMXShell::GetSize(LPCITEMIDLIST pidl)
{
    UINT cbTotal = 0;
    if (pidl)
    {
...
LPITEMIDLIST CSMXShell::Create(UINT cbSize)
{
    LPMALLOC lpMalloc;
    HRESULT  hr;
...
LPITEMIDLIST CSMXShell::ConcatPidls(LPCITEMIDLIST pidl1, LPCITEMIDLIST pidl2)
{
...
LPITEMIDLIST CSMXShell::CopyITEMID(LPMALLOC lpMalloc, LPITEMIDLIST lpi)
{
...
BOOL CSMXShell::GetName( LPSHELLFOLDER lpsf, LPITEMIDLIST lpi, DWORD dwFlags, LPTSTR lpFriendlyName )
{
   BOOL   bSuccess=TRUE;
   STRRET str;
...
LPITEMIDLIST CSMXShell::GetFullyQualPidl(LPSHELLFOLDER lpsf, LPITEMIDLIST lpi)
{
...
BOOL CSMXShell::DoTheMenuThing(HWND hwnd, LPSHELLFOLDER lpsfParent,
...
int CSMXShell::GetIcon(LPITEMIDLIST lpi, UINT uFlags)
{
	SHFILEINFO    sfi;
...
void CSMXShell::GetNormalAndSelectedIcons(LPITEMIDLIST lpifq, LPTV_ITEM lptvitem)
{
...
LONG CSMXShell::GetRegKey(HKEY key, LPCTSTR subkey, LPTSTR retdata)
{
    HKEY hkey;
...
HINSTANCE CSMXShell::ShellExec(LPCTSTR lpszExec, int nShow)
{
...
void CSMXShell::SplitPath(CString strFileName, char* szDrive, char* szDir, char* szFile, char* szExt)
{
#if defined( _UNICODE )
...
LPITEMIDLIST CSMXShell::DuplicateItem (LPMALLOC pMalloc, LPITEMIDLIST pidl)
{
    USHORT nLen;
    LPITEMIDLIST pidlNew;
...
LPITEMIDLIST CSMXShell::GetNextItem (LPITEMIDLIST pidl)
{
    USHORT nLen;
...
UINT CSMXShell::GetItemCount (LPITEMIDLIST pidl)
{
    USHORT nLen;
    UINT nCount;
...
BOOL CSMXShell::DoExplorerMenu (HWND hwnd, LPCTSTR pszPath, POINT point)
{

  return TrackItemIDContextMenu(pszPath, TPM_LEFTALIGN |
                        TPM_LEFTBUTTON | TPM_RIGHTBUTTON | TPM_RETURNCMD, &point, hwnd);
  /****
  ...
  LPMALLOC pMalloc;
    ...
    if (nCount) // nCount must be > 0
      ...
      while (nCount)
        ...
        //
        if (!SUCCEEDED (psfFolder->BindToObject (pidlNextItem, NULL, IID_IShellFolder, (LPVOID*)&psfNextFolder))) 
        {
...
void CSMXShell::Free (LPITEMIDLIST pIDL)
{
  GetMalloc()->Free(pIDL);
}
