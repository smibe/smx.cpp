...
BOOL COXGridList::SortColumn(int nColumn /* = 0 */)
  ...
  if (m_aCompareProc.GetSize() > nColumn && m_aCompareProc[nColumn])
	  return SortItems( (PFNLVCOMPARE) m_aCompareProc[nColumn], (LPARAM)(&gridListSortInfo));
  else
...
BOOL COXGridList::SetCurSel(int nSelectionItem, BOOL bSelect /* = TRUE */)
	...
	if (nSelectionItem == -1)
		...
		while (bSuccess && (nIndex <= nLastIndex))
			{
			bSuccess = SetItemState(nIndex++, bSelect ? LVNI_SELECTED | LVNI_FOCUSED: 0, LVNI_SELECTED | LVNI_FOCUSED);
			}
		return bSuccess;
		}
	return SetItemState(nSelectionItem, bSelect ? LVNI_SELECTED  | LVNI_FOCUSED : 0, LVNI_SELECTED | LVNI_FOCUSED);
	}
...
void COXGridList::SetRowText(LPDRAWITEMSTRUCT lpDIS, BOOL bShowItemSel)
  ...
  memset (&lvc, 0, sizeof(LVCOLUMN));
	lvc.mask = LVCF_FMT;
  COLORREF crPrvBk    = -1;
  COLORREF crPrevText = -1;
	while(iColumn < GetNumCols() && GetColumn(iColumn, &lvc))
		{
		  if ((int) lpDIS->itemID == m_nFoundItem && iColumn == m_nFoundSubItem)
      {
        crPrvBk    = pDC->SetBkColor(GetSysColor(COLOR_HIGHLIGHT));
        crPrevText = pDC->SetTextColor(GetSysColor(COLOR_HIGHLIGHTTEXT));
      }
...
void COXGridList::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) 
	...
	if (m_bAutoEdit && (nChar == VK_INSERT || nChar == VK_F2))
		{
...
void COXGridList::SetCompareProc(int nColumn,  PFNLVCOMPARE pfnCompare)
{
  if (m_aCompareProc.GetSize() <= nColumn)
    m_aCompareProc.SetSize(nColumn + 1);

  m_aCompareProc[nColumn] = pfnCompare;
}

EFindResult COXGridList::Find(const CString& str)
{
  EFindResult eResult = FIND_NotFound;
  CString strItem;
  CRect   rcPrev(0, 0, 0, 0);

  if (str.IsEmpty())
  {
    if (m_nFoundItem != -1 && m_nFoundSubItem != -1)
      rcPrev = GetRectFromSubItem(m_nFoundItem, m_nFoundSubItem);
  
    if (m_nFoundItem != -1)
      SetCurSel(m_nFoundItem);
  ...
  int nStartItem    = m_nFoundItem == -1 ? 0 : m_nFoundItem;
  int nStartSubItem = m_nFoundSubItem + 1;

  for (int i = nStartItem; i < GetItemCount(); i++)
  {
    for (int j = nStartSubItem; j < GetNumCols(); j++)
    {
      strItem = GetItemText(i, j);
      if (strItem.Find(str) != -1)
      {
        EnsureVisible(i, j, FALSE);
        if (m_nFoundItem != -1 && m_nFoundSubItem != -1)
        {
          rcPrev = GetRectFromSubItem(m_nFoundItem, m_nFoundSubItem);
        }
        ...
        m_nFoundSubItem = j;
        InvalidateRect(rcPrev);
        InvalidateRect(GetRectFromSubItem(i, j));
        return FIND_Found;
      }
      nStartSubItem = 0;
    }
  ...
  if (m_nFoundItem != -1)
  {
    eResult = FIND_EndOfList;

    if (m_nFoundSubItem != -1)
      rcPrev = GetRectFromSubItem(m_nFoundItem, m_nFoundSubItem);
  }
...
BOOL COXGridList::SortItems( PFNLVCOMPARE pfnCompare, DWORD dwData )
{
  return CListCtrl::SortItems(pfnCompare, dwData);
}

void COXGridList::ClearSelection()
{
	int nItem = -1;
  while (GetNextItem(nItem, LVNI_SELECTED) != -1)
  {
	  SetItemState(nItem, 0, LVNI_SELECTED | LVNI_FOCUSED);
  }
}
