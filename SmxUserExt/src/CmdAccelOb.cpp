...
// $RCSfile: CmdAccelOb.cpp,v $
// 
//------------------------------------------------------------------------------
//
// $Header: D:\\CVSRoot/SmxUserExt/src/CmdAccelOb.cpp,v 1.1 2002/09/20 09:37:07 GeH Exp $
//
********************************************************************************/
...
void CAccelsOb::GetString(CString& szBuffer)
{
	...
	TRACE("Internal error : (CAccelsOb::GetString) m_wKey invalid\n");
}
...
//
//
CCmdAccelOb::CCmdAccelOb(UINT nCommandID)
{
...
//
//
CCmdAccelOb::CCmdAccelOb(BYTE cVirt, UINT nCommandID, WORD wKey, bool bLocked)
{
	m_nCommandID = nCommandID;
...
CCmdAccelOb& CCmdAccelOb::operator=(const CCmdAccelOb& from)
{
	...
	m_nCommandID = from.m_nCommandID;
  m_strCommand = from.m_strCommand;
	m_strCommandInfo = from.m_strCommand;
...
UINT CCmdAccelOb::GetCommandID()
{
  return m_nCommandID;
}

void CCmdAccelOb::InitStrings()
{
  if (!m_strCommand.IsEmpty() || !m_strCommandInfo.IsEmpty())
  {
  ...
  if(!m_strCommand.LoadString(m_nCommandID))
  {
    ASSERT(false);
    m_strCommand     = _T("Unknown");
    m_strCommandInfo = m_strCommand;
  }
  else
  {
    int nIdx = m_strCommand.Find(TCHAR('\n'));
    ASSERT(nIdx != -1);
    if (nIdx != -1)
    {
      m_strCommandInfo = m_strCommand.Left(nIdx);
      m_strCommand     = m_strCommand.Mid(nIdx+1);
    }
  }
}

LPCTSTR CCmdAccelOb::GetCommandString()
{
...
LPCTSTR CCmdAccelOb::GetCommandInfoString()
{
  if (m_strCommandInfo.IsEmpty())
  {
    InitStrings();
  }
