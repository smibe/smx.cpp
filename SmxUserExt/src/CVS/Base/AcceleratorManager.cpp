...
bool CAcceleratorManager::UpdateWndTable()
{
  ...
  if (m_pWndConnected == NULL && m_pDocTemplate == NULL)
  {
  ...
  if (m_pWndConnected)
    hOldTable = m_pWndConnected->m_hAccelTable;
  else if (m_pDocTemplate)
    hOldTable = m_pDocTemplate->m_hAccelTable;
  ...
  {
    m_pDocTemplate->m_hAccelTable = hNewTable;
...
CAcceleratorManager& CAcceleratorManager::operator=(const CAcceleratorManager& accelmgr)
{
  ...
  {
    m_aAccelGroups.SetSize(accelmgr.m_aAccelGroups.GetSize());
    for (int i = 0; i < accelmgr.m_aAccelGroups.GetSize(); i++)
      m_aAccelGroups[i] = accelmgr.m_aAccelGroups[i];
  ...
  if (pAllGroups)
  {
    while (i +j < nCountAll + nCountAllGroups)
    {
      ...
      if (i >= nCountAll || pAllGroups[j] < m_pAll[i])
      {
        ...
        if (pAccel)
          AddAccel(pAccel[i].fVirt, pAllGroups[j], pAccel[i].key, false);
        else
