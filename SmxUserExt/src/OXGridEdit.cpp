...
// $RCSfile: OXGridEdit.cpp,v $
// 
// ABSTRACT:
//
//------------------------------------------------------------------------------
//
// $Header: D:\\CVSRoot/SmxUserExt/src/OXGridEdit.cpp,v 1.1 2002/10/12 07:20:54 GeH Exp $
//
********************************************************************************/
...
void COXGridEdit::SetWindowWidth(int nWidth)
	{
...
void COXGridEdit::OnWindowPosChanging(WINDOWPOS FAR* lpwndpos) 
	...
	if (m_bAdjustWindowWidth)
		...
		if (nTextLength != 0)
			...
			if (ptLast.x != 0)
				...
				GetWindowText(sText);
				CSize textSize(0, 0);
				VERIFY(GetTextExtentPoint(GetDC()->GetSafeHdc(), (LPCTSTR)sText, sText.GetLength(), &textSize));
	...
	// client area
	if (m_nRightParentBorder < lpwndpos->x + lpwndpos->cx)
...
void COXGridEdit::OnKeyUp(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
