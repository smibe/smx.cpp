...
// $RCSfile: SInstance.cpp,v $
// 
// ABSTRACT:
//
//------------------------------------------------------------------------------
//
// $Header: D:\\CVSRoot/SmxUserExt/src/SInstance.cpp,v 1.1 2002/09/20 09:37:08 GeH Exp $
//
********************************************************************************/
...
BOOL CInstanceChecker::TrackFirstInstanceRunning()
{
  ...
  {
    //Create the MMF
    int nMMFSize = sizeof(CWindowInstance) + 2 * sizeof(HANDLE);
 ...
 */
HWND CInstanceChecker::ActivatePreviousInstance(LPCTSTR pszCmdLine)
{
  if (m_hPrevInstance != NULL) //Whats happened to my handle !
  {
    ...
    if (pInstanceData != NULL) //Opening the MMF should work
    {
      ...
      //Write the command line to the buffer
      strncpy((char *) pInstanceData->buf, pszCmdLine, 256);
      ...
      else
      {
        ...
        wndPrev.SendMessage(WM_COMMAND, 0, 0);
...
void CInstanceChecker::GetCommandLine(CString& str)
{
  int nMMFSize = sizeof(CWindowInstance);
  HANDLE hInstance = ::OpenFileMapping(FILE_MAP_ALL_ACCESS, FALSE, MakeMMFFilename());

  if (!hInstance)
    return;

  CWindowInstance* pInstanceData = (CWindowInstance*) ::MapViewOfFile(hInstance, FILE_MAP_READ | FILE_MAP_WRITE, 0, 0, nMMFSize);
  ...
  if (pInstanceData != NULL) //Opening the MMF should work
  {
    ...
    //Write the command line to the buffer
    str = pInstanceData->buf;

    //Remove the command line string
    *pInstanceData->buf = NULL;
