...
IMPLEMENT_DYNAMIC(CSMXTreeCtrl, CTreeCtrl)

BEGIN_MESSAGE_MAP(CSMXTreeCtrl, CTreeCtrl)
	//{{AFX_MSG_MAP(CSMXTreeCtrl)
	ON_WM_PAINT()
	ON_WM_LBUTTONDOWN()
	ON_WM_ERASEBKGND()
	//}}AFX_MSG_MAP
...
HTREEITEM CSMXTreeCtrl::GetNextItem( HTREEITEM hItem )
{
	HTREEITEM hti = NULL;
...
// hItem        - The reference item
HTREEITEM CSMXTreeCtrl::GetPrevItem( HTREEITEM hItem )
{
	HTREEITEM       hti;
...
HTREEITEM CSMXTreeCtrl::GetLastItem( HTREEITEM hItem )
{
...
HTREEITEM CSMXTreeCtrl::FindItem(LPCTSTR lpszSearch, 
...
BOOL CSMXTreeCtrl::IsFindValid( HTREEITEM )
{
	return TRUE;
}

BOOL CSMXTreeCtrl::PreCreateWindow(CREATESTRUCT& cs) 
{
...
void CSMXTreeCtrl::SetItemFont(HTREEITEM hItem, LOGFONT& logfont)
{
	Color_Font cf;
...
void CSMXTreeCtrl::SetItemBold(HTREEITEM hItem, BOOL bBold)
{
	SetItemState( hItem, bBold ? TVIS_BOLD: 0, TVIS_BOLD );
}

void CSMXTreeCtrl::SetItemColor(HTREEITEM hItem, COLORREF color)
{
	Color_Font cf;
	if( !m_mapColorFont.Lookup( hItem, cf ) )
...
BOOL CSMXTreeCtrl::GetItemFont(HTREEITEM hItem, LOGFONT * plogfont)
{
	Color_Font cf;
	if( !m_mapColorFont.Lookup( hItem, cf ) )
		return FALSE;
...
BOOL CSMXTreeCtrl::GetItemBold(HTREEITEM hItem)
{
	return GetItemState( hItem, TVIS_BOLD ) & TVIS_BOLD;
}

COLORREF CSMXTreeCtrl::GetItemColor(HTREEITEM hItem)
{
...
void CSMXTreeCtrl::OnLButtonDown(UINT nFlags, CPoint point) 
{
	if(m_bMultiSelect)
	{
...
void CSMXTreeCtrl::ClearSelection()
{
	if(m_bMultiSelect)
	{
...
BOOL CSMXTreeCtrl::SelectItems(HTREEITEM hItemFrom, HTREEITEM hItemTo)
{
	if(m_bMultiSelect)
	{
...
HTREEITEM CSMXTreeCtrl::GetFirstSelectedItem()
{
	if(m_bMultiSelect)
	{
		for ( HTREEITEM hItem = GetRootItem(); hItem!=NULL; hItem = GetNextItem( hItem ) )
...
HTREEITEM CSMXTreeCtrl::GetNextSelectedItem(HTREEITEM hItem)
{
	if(m_bMultiSelect)
	{
...
HTREEITEM CSMXTreeCtrl::GetPrevSelectedItem(HTREEITEM hItem)
{
	if(m_bMultiSelect)
	{
...
BOOL CSMXTreeCtrl::OnEraseBkgnd(CDC* pDC) 
{
