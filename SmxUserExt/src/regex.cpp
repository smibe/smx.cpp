...
CRegExp* CRegExp::RegCompFileMask(const char *exp)
{
  CString str;
  const char *pChar = exp;
  char *pCopy = NULL;
  int len = 0;
  int nMeta = 0;
  ...
  pCopy = str.GetBuffer(nStrLen + nMeta + 1);

  pChar = exp;
  while (*pChar)
  {
    len = strcspn(pChar, REGEX_META);
    if (len < (int) strlen(pChar))
    {
      while (len)
      {
        if (*pChar == ';')
...
// int offset;
   ...
   *((short *)(scan+1)) = (short)((OP(scan) == REGEX_BACK) ? scan - val : val - scan);
}
