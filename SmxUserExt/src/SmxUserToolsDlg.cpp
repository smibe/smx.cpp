...
CSmxUserToolsDlg::CSmxUserToolsDlg(CWnd* pParent /*=NULL*/)
	: CSmxDialog(CSmxUserToolsDlg::IDD, pParent)
{
...
void CSmxUserToolsDlg::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CSmxUserToolsDlg, CSmxDialog)
...
BOOL CSmxUserToolsDlg::OnInitDialog() 
{
  ...
  {
    CSmxUserTool *pTool = m_pMgr->m_UserTools[i];
...
void CSmxUserToolsDlg::LoadUserTool(int nIdx)
{
  if (nIdx >= 0 && nIdx <= m_pMgr->m_UserTools.GetUpperBound())
  {
    CSmxUserTool *pUserTool = m_pMgr->m_UserTools[nIdx];
...
bool CSmxUserToolsDlg::StoreUserTool(int nIdx)
{
  if (nIdx >= 0 && nIdx <= m_pMgr->m_UserTools.GetUpperBound())
  {
    CSmxUserTool *pUserTool = m_pMgr->m_UserTools[nIdx];
...
void CSmxUserToolsDlg::OnNew() 
{
  ...
  if (nIdx != -1)
  {
    CSmxUserTool *pTool = new CSmxUserTool;
...
void CSmxUserToolsDlg::OnSelchangeUserTools() 
{
...
void CSmxUserToolsDlg::OnRemove() 
{
	int nIdx = m_List.GetCurSel();
  if (nIdx != -1)
  {
...
void CSmxUserToolsDlg::OnItemChanged(NMHDR* pNMHDR, LRESULT* pResult)
{
...
void CSmxUserToolsDlg::OnCancel()
{
  OnOK();
}

BOOL CSmxUserToolsDlg::PreTranslateMessage(MSG* pMsg) 
{
...
void CSmxUserToolsDlg::OnKillfocusMenuString() 
{
  CWnd *pWnd = GetFocus();
...
bool CSmxUserToolsDlg::CheckCommandString()
{
  CString    strCmd;
  CString    strExeFile;
...
bool CSmxUserToolsDlg::UpdateMenuText()
{
	CString str;
...
void CSmxUserToolsDlg::OnKillfocusCommand() 
{
  CWnd *pWnd = GetFocus();
...
void CSmxUserToolsDlg::OnParameter() 
{
	CSelectVarDlg dlg;
  CRect rcButton;
...
void CSmxUserToolsDlg::OnInitialDir() 
{
	CSelectVarDlg dlg;
  CRect rcButton;
...
void CSmxUserToolsDlg::OnBatchFile() 
{
...
void CSmxUserToolsDlg::OnBatchFileName() 
{
  CString strFileName;
...
void CSmxUserToolsDlg::OnCommand() 
{
  CString strFileName;
