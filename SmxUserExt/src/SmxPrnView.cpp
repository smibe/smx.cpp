...
void CSmxPrnView::CalcTextRect(CRect&rcText, LPCTSTR pszText, UINT nFormat)
{
  ...
  LPtoDC(rcText);
  if (m_pDC->m_hAttribDC)
  {
    CDC *pDC = CDC::FromHandle(m_pDC->m_hAttribDC);
    pDC->DrawText(pszText, -1, rcText, nCalcFormat);
  }
  else
  {
...
{
  if (m_nCurrentLineHeight)
    return m_nCurrentLineHeight;

  CRect rcText;
  CalcTextRect(rcText, _T("jP"), DT_LEFT | DT_SINGLELINE);
  m_nCurrentLineHeight = rcText.Height();
  return m_nCurrentLineHeight;
}
...
void CSmxPrnView::OnPrepareDC( CDC* pDC, CPrintInfo* pInfo)
{
  ...
  else
  {
    ...
    m_rcClient -= m_rcClient.TopLeft();

    //Adjust the borders
    CSize szPaper;
    szPaper.cx = pDC->GetDeviceCaps(PHYSICALWIDTH);
    szPaper.cy = pDC->GetDeviceCaps(PHYSICALHEIGHT);
    pDC->DPtoLP(&szPaper);
    m_rcClient.right = m_rcClient.left + szPaper.cx - (m_nPaperLeftMargin + m_nPaperRightMargin);
    m_rcClient.bottom = m_rcClient.top + szPaper.cy - (m_nPaperTopMargin + m_nPaperBottomMargin);
  }
...
int CSmxPrnView::GetPaperHeight()
{
  ...
  if (nPaperHeight == 0)
  {
    ...
    CSize szPaperSize;
    szPaperSize.cx = dc.GetDeviceCaps(PHYSICALWIDTH);
    szPaperSize.cy = dc.GetDeviceCaps(PHYSICALHEIGHT);
    dc.DPtoLP(&szPaperSize);
    nPaperHeight = szPaperSize.cy;
  }
  return nPaperHeight;
}
...
void CSmxPrnView::LPtoHiMetric(CPoint *pPoint)
{
  ASSERT(m_pDC->GetWindowOrg() == CPoint(0, 0));
  //GPMASSERT -> If the window origin is not at zero, we do not know if the 
  //             window origin should be changed in MM_HIMETRIC coordinates as well!!
...
void CSmxPrnView::OnFilePrintPreview()
{
  if (GetPageSetupSection())
  {
    CSmxPgSetupDlg dlg(0, this);
    dlg.m_strRegKey = GetPageSetupSection();
    dlg.LoadFromProfile();

    m_nPaperTopMargin = (int) (dlg.m_ebTop.GetValue() * 1000.0 + 0.5);
    m_nPaperBottomMargin = (int) (dlg.m_ebBottom.GetValue() * 1000.0 + 0.5);
    m_nPaperLeftMargin   = (int) (dlg.m_ebLeft.GetValue() * 1000.0 + 0.5);
    m_nPaperRightMargin  = (int) (dlg.m_ebRight.GetValue() * 1000.0 + 0.5);
    SetPageOrientation(dlg.m_bLandscape);

    m_nNofColumns = (int) (dlg.m_ebNofCols.GetValue());
    m_nFirstCol = (int) (dlg.m_ebFirstCol.GetValue());
  ...
  if (pWnd && pWnd->IsKindOf(RUNTIME_CLASS(CFrameWnd)))
  {
    ...
    if (pFrame->m_hAccelTable)
    {
      m_hSavedFrmAccelTable = pFrame->m_hAccelTable;
...
void CSmxPrnView::OnEndPrintPreview( CDC* pDC, CPrintInfo* pInfo, 
                               POINT point, CPreviewView* pView )
{
  ...
  {
    CFrameWnd *pFrame = (CFrameWnd *) pWnd;
    if (pFrame->m_hAccelTable == NULL && m_hSavedFrmAccelTable != 0)
    {
      pFrame->m_hAccelTable = m_hSavedFrmAccelTable;
      m_hSavedFrmAccelTable = NULL;
    }
  }
  CScrollView::OnEndPrintPreview( pDC, pInfo, point, pView );
}

void CSmxPrnView::PrintXMLText(CXMLElement *pElt, UINT nFormat)
{
  if (!pElt)
    return;
  PrintText(pElt->GetCompleteText(), nFormat);
}

void CSmxPrnView::UpdatePageSetup()
{
  if (GetPageSetupSection())
  {
    CSmxPgSetupDlg dlg(0, this);
